...
The **You say** column is the primary entry path — CONSTRUCT is driven by conversation, and
every row below works by asking for it in natural language.

The **CLI** column is the same capability reached directly from a shell, for scripting,
automation, and CI. Not every conversational entry has one: rows marked `skill-only` are
handled by a skill with no shell equivalent, and an empty cell means neither exists yet.
Nothing in this column is aspirational — every invocation here is checked against the live
command surface by `tests/contract/test_doc_command_references.py`, which is the authority on
what actually ships. Tokens in angle brackets are placeholders for you to substitute.
...
state in the `--json` payload to judge the outcome — the process exit code alone does not
carry the verdict.
...
| You say | What happens | CLI |
|---------|-------------|-----|
| "write {topic}" | Draft a document from accumulated knowledge | skill-only |
| "publish" | List and manage published outputs in `publish/` | skill-only |

### Views & server

| You say | What happens | CLI |
|---------|-------------|-----|
| "scaffold views" | One-time SPA template setup | skill-only |
| "build views" | Vite production build | skill-only |
| "update views" / "refresh data" | Parse workspace → JSON for dashboard | `construct views generate` |
| "check the generated views data" | Validate the generated dashboard data against its schema | `construct views validate` |
| "start" / "show views" | Start local server on port 3001–3009 | skill-only |
| "stop" | Stop local server | skill-only |
| "reset views" | Remove all views artifacts — clean slate | skill-only |
