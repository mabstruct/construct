---
description: "Scaffold the CONSTRUCT views SPA from template (one-time setup). Use when user says 'scaffold the views', 'set up the views app', 'initialize CONSTRUCT views'."
...
```
<install-root>/.claude/skills/views-scaffold/template/
```

Verify it exists:

```bash
test -d <install-root>/.claude/skills/views-scaffold/template && echo OK || echo MISSING
```

If missing → fail with: `Scaffold template missing at .claude/skills/views-scaffold/template/. Reinstall the CONSTRUCT skill set.`
...
cp -a <install-root>/.claude/skills/views-scaffold/template/. <install-root>/views/src/
```
...
✓ Scaffolded views at <install-root>/views/src/
  Template: .claude/skills/views-scaffold/template/
  Version: <VERSION>
