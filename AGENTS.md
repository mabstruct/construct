...
- YAML — workspace canonical config files (`.construct/governance.yaml`, `domains.yaml`); `src/construct/llm/config.yaml` is the LLM configuration authority; `.construct/model-routing.yaml` is deprecated and inert, retained only so `REQUIRED_PATHS` workspace-contract checks stay stable
...
## Configuration
- `src/construct/llm/config.yaml` — **the LLM configuration authority**: provider selection and task-tier routing (frontier/workhorse/lightweight) are read from here
- `.construct/model-routing.yaml` — **deprecated and inert**; nothing reads it. Retained solely so `REQUIRED_PATHS` workspace-contract checks stay stable (template at `CONSTRUCT-CLAUDE-impl/construct/templates/model-routing.yaml`). Use `src/construct/llm/config.yaml` instead.
...
- Location: `src/construct/cli.py` and `src/construct/mcp/` (both live); planned `src/construct/api/`
- Contains: a Typer CLI organised as capability groups — `ask`, `bridge`, `card`, `curation`, `daily`, `ingest`, `knowledge` (with `card` and `connection` sub-groups), `research`, `spike`, `tag`, `views`, `workflow` — alongside the top-level leaves `help`, `init`, `mcp`, `status`, `validate`; MCP tool definitions auto-discovered from the capability registry rather than hand-written; planned HTTP routes. The exact live surface is asserted mechanically by `tests/contract/test_doc_command_references.py` — treat that test as the authority, not any count written in prose.
