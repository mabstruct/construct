...
1. **D-03 exact semantics (escalate handling).** *What we know:* the child `approve_all` path writes recommended promote/connection/archive and records escalates as un-written (`escalated`), then the run completes. *What's unclear:* whether the user wants escalates literally left on an open, resumable checkpoint (which forbids applying anything else that pass — contradicts D-01/D-02) vs. "not written + surfaced as a pending count" (compatible). *Recommendation:* adopt the compatible reading (auto-apply recommendations, surface pending-escalation count, escalates addressable via a later `curation review --run-id <...>-curation` only if that thread is intentionally *not* resumed). **Surface this to discuss-phase before locking the plan.**
2. **`daily.inspect` persistence location/format.** *What we know:* thin composition has no parent checkpoint; a small receipt is needed. *Recommendation:* `.construct/workflow/daily/<run_id>.json` holding the serialized `DailyRunResult`; missing → `failed`. Confirm the path convention with the planner (matches `.construct/workflow/*-run.sqlite` sibling convention).
3. **Auto-apply flag surface.** *What we know:* default is auto-apply (D-02); discretion allows an opt-out flag. *Recommendation:* ship default-on with **no** flag in this phase (deferred `--skip-research`/focus flags stay out per Deferred Ideas); revisit if an attended mode is ever wanted.
